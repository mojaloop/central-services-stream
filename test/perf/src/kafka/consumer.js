    ...
    return new Promise((resolve, reject) => {
      ...
      if (messages) { // check if there is a valid message comming back
        ...
        if (Array.isArray(messages) && messages.length != null && messages.length > 0) {
          Logger.info(`[cid=${message.value.content.batchId}, batchSize=${messages.length}, topicName=${topics}] ~ Consumer::perf::consumeFunction - Batch - START`)
          ...
          Logger.info(`[cid=${message.value.content.batchId}, batchSize=${messages.length}, topicName=${topics}] ~ Consumer::perf::consumeFunction - Batch - END`)
        } else {
        ...
        Logger.info(`[cid=${message.value.content.batchId}, messageBatch=1, messageSize=${message.length}, topicName=${topics}, tid=${message.value.content.id}] ~ Consumer::perf::consumeFunction - Message - END`)
        resolve(true)
      } else {
        resolve(false)
      }
